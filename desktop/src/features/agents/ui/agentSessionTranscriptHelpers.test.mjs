...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // section is a "Hyperbuzz event", there is no event content to surface, so
  // userText is empty and the title falls through to "Hyperbuzz event".
  ...
  assert.equal(result.userTitle, "Hyperbuzz event");
...
test("parsePromptText defaults the title to 'Hyperbuzz event' when no kind is present", () => {
...
test("parseSystemPromptSections extracts Team Instructions with Core Memory and Channel Canvas (full 5-section shape)", () => {
  ...
  const framed = [
    "[Base]",
    "You are a helpful AI assistant running in Hyperbuzz.",
    "",
    "[System]",
  ...
  assert.deepEqual(sections, [
    {
      title: "Base",
      body: "You are a helpful AI assistant running in Hyperbuzz.",
    },
    {
      title: "System",
