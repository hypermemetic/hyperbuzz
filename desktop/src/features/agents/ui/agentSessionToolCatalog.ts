...
export function getBuzzToolInfo(title: string): BuzzToolInfo | null {
  ...
  if (name.includes("workflow") || name === "approve_step") {
    return {
      icon: Workflow,
      label: isRead
        ? "Reads workflow state from Hyperbuzz."
        : "Updates workflow state in Hyperbuzz.",
  ...
  if (
    ...
    return {
      icon: Hash,
      label: isRead
        ? "Reads channel context from the Hyperbuzz relay."
        : "Changes channel state in the Hyperbuzz relay.",
      tone: isWrite ? "write" : "read",
    };
  }
  if (
    ...
    return {
      icon: Users,
      label: isRead
        ? "Reads Hyperbuzz identity or presence data."
        : "Updates Hyperbuzz identity or membership data.",
  ...
  if (name.includes("search") || name === "get_feed") {
    return {
      icon: Search,
      label: "Searches relay-visible Hyperbuzz history.",
      tone: "read",
    };
  }
  if (
    ...
    return {
      icon: Send,
      label: "Publishes relay-visible Hyperbuzz activity.",
      tone: "write",
    };
  }
  ...
  return {
    icon: MessageSquare,
    label: isRead ? "Reads from Hyperbuzz." : "Writes to Hyperbuzz.",
    tone: isWrite ? "write" : "read",
  };
}
