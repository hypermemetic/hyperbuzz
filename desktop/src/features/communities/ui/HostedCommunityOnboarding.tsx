      ...
      if (released.error) {
        throw new Error(
          hostedCommunityErrorMessage(
            ...
            "Could not disconnect the account's previous Hyperbuzz identity.",
          ),
        );
      }
      ...
      if (bound.error) {
        ...
        throw new Error(
          bound.error.code === "pubkey_already_bound"
            ? "This device's Hyperbuzz identity belongs to a different Builderlab account and can't be moved from here. Sign out, then sign in with the account that already owns this identity."
            : hostedCommunityErrorMessage(
                bound.error,
        ...
        <div className="mx-auto flex w-full max-w-sm flex-col items-center py-2 text-center">
          ...
          {!auth ? (
            <>
              ...
              {/* Quiet breadcrumb: Hyperbuzz itself is open source; this hosted
              ...
              <p className="mt-6 w-full border-t border-foreground/10 pt-4 text-xs leading-5 text-foreground/45">
                Hyperbuzz is open source. Builderlab hosts the relay for this
                account.
              </p>
            </>
          ) : !identity ? (
            <>
              <DialogTitle className="text-xl font-medium text-foreground">
                Finish connecting Hyperbuzz
              </DialogTitle>
              <DialogDescription className="mt-2 text-sm leading-6 text-foreground">
                Your Builderlab account
                {auth.email ? ` (${auth.email})` : ""} is ready. Connect this
                device’s Hyperbuzz identity to finish setup. Your private key
            ...
            <>
              <DialogTitle className="text-xl font-medium text-foreground">
                This account uses a different Hyperbuzz identity
              </DialogTitle>
              <DialogDescription className="mt-2 text-sm leading-6 text-foreground">
                This account is connected to another Hyperbuzz identity.
    ...
    <div className="flex min-h-[calc(100dvh-15.625rem)] w-full max-w-[920px] flex-col items-center text-center">
      ...
      <p className="mx-auto mt-2 max-w-[560px] text-sm leading-6 text-foreground">
        {hasCommunities
          ...
          : "Claim a Hyperbuzz address to get started."}
      </p>
