...
export function HostedCommunitiesSettingsCard() {
  ...
  const loadAccount = React.useCallback(async () => {
    ...
    if (
      ...
      // Hyperbuzz identity yet — that's the connect-card empty state, not an error to
      ...
      throw new Error(
        errorMessage(
          ...
          "Could not load the connected Hyperbuzz identity.",
        ),
      );
    }
  ...
  const connectIdentity = () =>
    run("Connecting Hyperbuzz identity…", async () => {
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            ...
            "Could not connect the Hyperbuzz identity.",
          ),
        );
      }
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            response.error,
            response.correlation_id,
            "Could not unpair the Hyperbuzz identity.",
          ),
        );
      }
      setIdentity(null);
      ...
      if (released.error) {
        throw new Error(
          errorMessage(
            ...
            "Could not release the previously connected Hyperbuzz identity.",
          ),
        );
      }
      ...
      if (bound.error) {
        ...
        throw new Error(
          bound.error.code === "pubkey_already_bound"
            ? "This device's Hyperbuzz identity is already reserved by another Builderlab account, so it can't be connected here. Sign in with that account, or transfer the identity there first."
            : errorMessage(
                bound.error,
                bound.correlation_id,
                "Could not connect this device's Hyperbuzz identity.",
              ),
        );
      }
      ...
      if (availabilityResponse.error || !availabilityResponse.available) {
        ...
        throw new Error(
          errorMessage(
            ...
            "That Hyperbuzz address is already taken.",
          ),
        );
      }
      const response = await invoke<CommunityMutationResponse>(
  ...
  return (
    <section className="space-y-6" data-testid="hosted-communities-settings">
      <SettingsSectionHeader
        ...
        description="Hyperbuzz works with any relay. This page is only for relay hosting provided by Block — sign in with a Builderlab account to create and manage Block-hosted communities. Builderlab sign-in is used on this page alone."
      />
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-border/70 p-5">
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            ...
            Hyperbuzz. You can use every other part of the app without signing
            in.
          </p>
          <Button
            className="mt-4"
          ...
          {!identity ? (
            <div className="rounded-xl border border-amber-500/40 bg-amber-500/5 p-5">
              <h3 className="font-medium">
                Link this account to your Hyperbuzz identity
              </h3>
              <p className="mt-2 text-sm text-muted-foreground">
                This Builderlab account isn&apos;t linked to a Hyperbuzz
                identity yet. Connect this device&apos;s key to create and own
                communities under it — Hyperbuzz signs a one-time challenge
              ...
              >
                ...
                {action ?? "Connect Hyperbuzz identity"}
              </Button>
            </div>
          ) : identityMismatch ? (
            <div className="rounded-xl border border-amber-500/50 bg-amber-500/5 p-5">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <h3 className="font-medium">
                    This account is connected to a different Hyperbuzz identity
                  </h3>
                  <p className="mt-2 text-sm text-muted-foreground">
                    Your Builderlab account owns communities under another
                    Hyperbuzz key, so connecting them here would join a relay
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/70 p-4">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <CheckCircle2 className="h-4 w-4 text-emerald-500" /> Hyperbuzz
                identity connected
                {identity.npub ? (
...
function UnpairIdentityButton({
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Unpair this Hyperbuzz identity?</AlertDialogTitle>
          <AlertDialogDescription>
            Your Builderlab account will no longer be connected to this
            Hyperbuzz key. You can reconnect any key later, but community
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            member. The recipient needs a connected Hyperbuzz identity first,
